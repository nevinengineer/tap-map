...
const EstablishmentMap = ({ latitude, longitude }) => {
  //const { latitude, longitude, timestamp, accuracy, positionError } = usePosition();
  const [establishmentId, setEstablishmentId] = useState(null);
  // const [itemSelected, setItemSelected] = useState(handleBrewItemSelected)
  ...
  const handleItem = (item) => {
    ...
    item.on_taps.forEach(element => {
      //console.log(element.establishment_id)
      tapArray.push(element.establishment_id)
    });
    //setSelectedItem(item);
    //console.log(item.on_taps)

    setOnTapArray(tapArray)
    //selected()
    //console.log(onTapArray)
    //console.log(establishments)
  };

  const handlePopup = index => {
    console.log(index.establishment_id)
  }
  ...
  return (
    <div>
      <SearchAndResults onSelectedItem={handleItem}></SearchAndResults>
            ...
            {establishments.map((index) => (
              ...
              >
                <EstablishmentPopup
                  ...
                  active={onTapArray.includes(index.establishment_id) ? true : false}
                ></EstablishmentPopup>
              </Marker>
            ))}
          </Map>
